...
|---|---|---|---|
| Target MySQL migration and restore | Target-required | `scripts/verify_mysql.ps1` contains the migration, restart, replay, backup, concurrency, and native dump/restore assertions. The 2026-08-29 local attempt stopped at its fail-fast prerequisite probe because the configured `dev@localhost` preview credentials were rejected (`ERROR 1045`); no persistence assertion was claimed from that run. | Restore working preview credentials and run the full script, then have the deployment owner repeat the same checks against the target database, including a known-good restore and integrity comparison. |
...
| Richer route and market inspection | Implemented surface | The online sidebar keeps compact authoritative road availability/risk, open orders, and the protected-law boundary, while the visible `Inspect` control opens route names/status/condition/risk, the first stock and price notes, and the latest event's cause, exact choices, and outcome state from the same server projections. | Playtest the detail wording and information density; adjust the surface if player evidence shows that detail competes with recovery, comparison, or chronicle controls. |
