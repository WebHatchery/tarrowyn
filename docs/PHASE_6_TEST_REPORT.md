...
The Phase 5 server restart and replay regressions now live in
`repository/phase5/tests/replay_restart.rs`, reducing the phase test root from
544 to 356 lines while preserving accepted and rejected regional mutation
replay plus authentication replay across repository restarts. The focused
replay-restart filter passes (3 tests, with 439 unrelated tests filtered),
along with server-package clippy with warnings denied, package formatting,
`git diff --check`, and the Rust file-size scan. No publisher or full workspace
gate is repeated because this is test-source organization maintenance, and no
new external or deferred work is opened.
...
The deleted-account governance replay privacy correction was validated on
2026-08-31. Cached governance snapshots now receive the same office, proposal,
decision, and tax-identity anonymization as live deletion cleanup; the focused
exact regression filter passed 1 test:
`account_deletion_anonymises_governance_replays_kept_by_another_identity`.
Server-package formatting, clippy, Rust file-size, and diff checks passed. No
...
The orphaned-chronicle privacy correction was validated on 2026-08-31. Restore
normalization now collects names from orphaned structured public records before
scrubbing, then anonymises matching Phase 3 recent/archive, Phase 5 settlement,
and retained chronicle-event text without replacing names still used by active
