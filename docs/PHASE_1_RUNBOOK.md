...
tick loop. The default tick is 250 ms and advances the world clock by 0.25
world seconds, so an 80-minute in-game day lasts 80 real minutes. Set
`TARROWYN_WORLD_SECONDS_PER_TICK` explicitly when a faster deterministic
fixture is needed.
...
| `TARROWYN_WORLD_SECONDS_PER_TICK` | `0.25` | World-clock seconds advanced per server tick; defaults to the tick interval and can be overridden for fixtures. |
...
release contract in `protocol/src/lib.rs` has `PROTOCOL_VERSION` set to `7`.
